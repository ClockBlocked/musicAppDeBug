...
const ui = {
  ...
  setLoadingState: (loading) => {
    const nowPlayingArea = $byId(NAVBAR.nowPlayingArea);
  ...
  updatePlayPauseButtons: () => {
    const navControls = {
      playIcon: $byId(NAVBAR.playIcon),
      pauseIcon: $byId(NAVBAR.pauseIcon),
    };

    const playerControls = {
      playIcon: $byId(MUSIC_PLAYER.playIcon),
      pauseIcon: $byId(MUSIC_PLAYER.pauseIcon),
    };

    // Toggle navbar icons based on playing state
    if (navControls.playIcon && navControls.pauseIcon) {
      navControls.playIcon.style.display = appState.isPlaying ? "none" : "block";
      navControls.pauseIcon.style.display = appState.isPlaying ? "block" : "none";
    }

    // Toggle popup icons based on playing state
    if (playerControls.playIcon && playerControls.pauseIcon) {
      playerControls.playIcon.style.display = appState.isPlaying ? "none" : "block";
      playerControls.pauseIcon.style.display = appState.isPlaying ? "block" : "none";
    }
  },
...
const eventHandlers = {
  ...
  bindControls: () => {
    ...
    const controlElements = {
      [NAVBAR.playPause]: player.toggle, // Play/pause button in navbar
      [NAVBAR.previous]: player.previous, // Previous track button in navbar
      [NAVBAR.next]: player.next, // Next track button in navbar
      [NAVBAR.nowPlayingArea]: popup.toggle, // Now playing area click to open popup
    };
...
window.debugMusicPlayer = () => {
  ...
  const musicPlayer = $byId(MUSIC_PLAYER.root);
